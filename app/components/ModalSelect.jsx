...
export default function ModalSelect(props) {
  ...
  // 🚫 Do NOT trigger state until we are sure window exists
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className={`bg-white w-full max-w-2xl rounded-2xl py-6 px-4 m-3 shadow-2xl border border-white/30 animate-fadeUp ${shakeModal ? 'animate-shake' : ''}`}>
        <Image src="/images/logo.png" alt="logo" width={300} height={300} className="w-auto h-24 sm:h-28 md:h-40 mx-auto mb-6" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-4">
          <div onClick={() => handleClick('Corporate-Gifting')} className="relative cursor-pointer rounded-xl overflow-hidden shadow-lg w-full h-[160px] sm:h-[200px] group hover:scale-[1.03] transition-transform">
          ...
          <div onClick={() => handleClick('Cloud Kitchen')} className="relative cursor-pointer rounded-xl overflow-hidden shadow-lg w-full h-[160px] sm:h-[200px] group hover:scale-[1.03] transition-transform">
        ...
        <p className="text-center text-gray-700 text-xs sm:text-sm">Tap an option above to continue</p>
      </div>
    </div>
  );
}
