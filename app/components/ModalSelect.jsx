...
export default function ModalSelect() {
  ...
  const [openSelectState, setOpenSelectState] = useState(true);
  ...
  const handleOnClose = (e) => {
    if (e.target.id === 'modal-container') {
      if (!selectedState) {
        setError('Please select any state');
        setShakeModal(true);
  ...
  const handleClick = (option) => {
    ...
    if (option === 'Odisha') {
      router.push('/chocolates');   // 👈 Chocolates page
    } else if (option === 'Chhattisgarh') {
  ...
  return (
    ...
    >
      <div
        className={`bg-white/90 w-full max-w-2xl rounded-2xl py-6 px-4 m-3 shadow-2xl
        backdrop-blur-xl border border-white/30 transition-all duration-200
        ...
        <div className='flex flex-col md:flex-row justify-center items-center gap-6 mb-4'>
          ...
          <div
            ...
            className='relative cursor-pointer rounded-xl overflow-hidden shadow-lg 
                       w-[280px] h-[200px] group hover:scale-[1.03] transition-transform'
          >
            ...
            <div className='absolute inset-0 bg-black/40 group-hover:bg-black/20 
                            flex justify-center items-center text-white text-3xl font-semibold'>
              Chocolates
            </div>
          </div>
